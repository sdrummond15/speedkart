...
function __sigplusSearch(url, preview_url, width, height, title, summary) {
	...
	*/
	...
	if (resultlink) {
		if (title) {
			resultlink.dataset['title'] = title;
		}
		if (summary) {
			resultlink.dataset['summary'] = summary;
		}

		let resulttext = following(closest(resultlink, '.result-title'), '.result-text');
		if (resulttext) {
			resulttext.classList.add('sigplus-search-result');
